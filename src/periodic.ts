...
function weekDay(client: any) {
    //#region Daily
        if((new Date).getHours() == 8) { if(!stCtrl[1]) { checkIfCumple(client); checkIfAFK(client); stCtrl[0] = true; } } else { stCtrl[0] = false; }
    //#endregion
    switch ((new Date).getDay()) {
        case 0: {
            if(((new Date).getHours() == 23)) { if(!stCtrl[2]) { nextFuhrer(client); isAFK(client); stCtrl[1] = true; } }  else { stCtrl[1] = false; } //Baja Rango/Week & cambio Fuhrer
            break;
        } 
    }
}
